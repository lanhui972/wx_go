...
export default class Dongtu extends Dom {
    ...
    curr(n){
        this.attr.nCurrImg = n;
        this.attr.imgs[n].n = this.attr.imgs[n].min;
    }
    xgAttr() {
        if (un(this.attr.nCurrImg) || undefined === this.attr.imgs){
            //return;
        }
        if (!un(this.attr.imgs[this.attr.nCurrImg].width) && this.attr.imgs[this.attr.nCurrImg].width!=this.width()){
            this.x(this.x() + (this.width() - this.attr.imgs[this.attr.nCurrImg].width*bli) / 2);
            this.width(this.attr.imgs[this.attr.nCurrImg].width * bli);
        }
        if (!un(this.attr.imgs[this.attr.nCurrImg].height) && this.attr.imgs[this.attr.nCurrImg].height != this.height()) {
            this.y(this.y() + (this.height() - this.attr.imgs[this.attr.nCurrImg].height * bli)/2 );
            this.height(this.attr.imgs[this.attr.nCurrImg].height * bli);
        }
        
        if (!un(this.attr.imgs[this.attr.nCurrImg].dongsu)) {
            //p(this.attr.imgs[this.attr.nCurrImg].dongsu);
            this.dongsu = this.attr.imgs[this.attr.nCurrImg].dongsu;
        }else {
            this.dongsu = this.mrDongSu;
        }
